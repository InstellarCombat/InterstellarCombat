 ...
 * This class sets up the state of the game while running (after menu panel)
 * It initializes all the things the player see's while playing.
 */
public class InGameState extends AbstractAppState implements PhysicsCollisionListener {
    ...
    public void update(float tpf) {
      ...
      // modify scene graph...
      // call some methods...   	
    	System.out.println(otherShip.getRigidBodyControl().getLinearVelocity());
        if (counter == 1)
        {
      	  up = false;
      	  counter--;
  ...
  private void prepareOtherHealthBar() {
  ...
  private void updateOtherHealthBar() {
  ...
  @Override
  /**
   * Detects any collisions in the InGameState 
   */
