...
import mygame.InterstellarCombat;
import mygame.Main;
...
public class OptionPanel extends JPanel {
    ...
    private void createStartCoords () {
    	if (w.getInfo().isServer()) {
    		w.getInfo().setCoords(new Vector3f(0,0,5));
    	} else {
    		w.getInfo().setCoords(new Vector3f(0,0,-5));
    	}
    }
    private void startApp() {
        ...
        w.getCanvas().add(canvas, BorderLayout.CENTER);
    }
    ...
    class optionListener implements MouseListener {
		public void mouseClicked(MouseEvent e) {
			...
			if(b == MouseEvent.BUTTON1 && rec.contains(xMouse, yMouse)) {
				...
				createStartCoords();
				((InterstellarCombat)w.getApp()).setHandler(w.getNetManager().createHandler(w.getInfo()));
				startApp();
				w.getInfo().startGame(true);
				w.changePanel("play");
